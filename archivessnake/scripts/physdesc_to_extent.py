...
class PhysdescToExtent(object):
    ...
    def run(self):
        ...
        for ao in archival_objects:
            ...
            if physdesc_notes and len(physdesc_notes) <= 12:
                extent_list = physdesc_notes.strip("()").split(" ")
            elif physdesc_notes and len(physdesc_notes) > 12:
                # then write method to log physdesc to csv if len > 12 ?
                # do we want this in the run method or another one?
