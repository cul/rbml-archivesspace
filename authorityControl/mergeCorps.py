...
with open('input_corps.csv', 'rb') as csvfile:
        reader = csv.reader(csvfile, delimiter=',', quotechar='"')
        for row in reader:
        	#take target AS ID from row 0
        	t = {}
        	t = {}
        	t['ref'] = '/agents/corporate_entities/' + row[0]
        	#take victim 1 AS ID from row 1
        	v1 = {}
        	v1['ref'] = '/agents/corporate_entities/' + row[1]
        	victimList = [v1]
        	#test for second victim AS ID in row 2; if exists, update list
        	if row[2] != "":
        		v2 = {}
        		v2['ref'] = '/agents/corporate_entities/' + row[2]
        		victimList = [v1,v2]
        	#create record	
        	record = {}
        	record['target'] = t
