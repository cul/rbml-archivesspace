...
def ASAuthenticate(user, baseURL, password):
    ...
    if session_token != '':
        ...
        try:
            auth = requests.post(baseURL + '/users/'+user +
...
def getArchivalObjectByRef(repo, ref):
    ...
    lookup = requests.get(baseURL + endpoint,
...
def getResourceByID(repo, ref):
    ...
    output = requests.get(baseURL + endpoint,
                          headers=headers, params=params).json()
    output = json.dumps(output)
    return output
...
def getDigitalObjectFromParent(repo, ref):
    ...
    the_dao_refs = [inst['digital_object']
                    for inst in the_parent['instances'] if 'digital_object' in inst]
...
def getEAD(repo, asid):
    ...
    headers = ASAuthenticate(user, baseURL, password)
    endpoint = '/repositories/' + str(repo) + '/resource_descriptions/' + str(
        asid) + '.xml' + '?include_unpublished=true&include_daos=true'
...
def getArchivalObjectChildren(repo, asid):
    ...
    headers = ASAuthenticate(user, baseURL, password)
    endpoint = '/repositories/' + \
        str(repo) + '/archival_objects/' + str(asid) + '/children'
...
def getByDate(repo, date, date_type='ctime', comparator='equal', filter=None, fields=['id', 'create_time', 'title']):
    ...
    aqparams = '{"query":{"field":"' + date_field + '","value":"' + date + '","comparator":"' + \
        comparator + '","jsonmodel_type":"date_field_query"},"jsonmodel_type":"advanced_query"}'
...
def getSearchResults(repo, query_params):
    ...
    endpoint = '//repositories/' + str(repo) + '/search?page=' + str(
        pageno) + '&page_size=' + str(page_size) + '&aq=' + query_params
...
def daosRecurse(repo, asid):
    ...
    for a_child in x:
        ...
        the_dao_refs = [inst['digital_object']['ref']
                        for inst in a_child['instances'] if 'digital_object' in inst]
...
def suppressEnumerationValue(asid, mode='suppress'):
    ...
    headers = ASAuthenticate(user, baseURL, password)
    endpoint = '/config/enumeration_values/' + \
